    ...
    <section id="home" className="relative min-h-screen flex items-center">
      ...
      <div className="relative z-10 container mx-auto px-4 py-20">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          {/* Right Column - Business Directions */}
          <div className="animate-industrial-fade-in">
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20">
              <h3 className="text-2xl font-bold text-white mb-6">Направления деятельности</h3>
              <div className="space-y-4">
                {businessDirections.map((direction, index) => (
                  <div 
                    key={index}
                    className="flex items-start space-x-4 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-300 cursor-pointer"
                  >
                    <div className="flex-shrink-0">
                      <direction.icon className={`w-8 h-8 ${direction.color}`} />
                    </div>
                    <div>
                      <h4 className="font-semibold text-white mb-1">{direction.title}</h4>
